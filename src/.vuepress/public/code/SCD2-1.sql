...
WHEN MATCHED AND (DimProducts.EndDate=CAST('21000101' AS datetime)
  ...
  UPDATE SET
    EndDate=getdate()
WHEN NOT MATCHED BY SOURCE AND (DimProducts.EndDate=CAST('21000101' AS datetime)) THEN
  UPDATE SET
    EndDate=getdate();
GO
...
target AS (
  ...
  WHERE DimProducts.EndDate=CAST('21000101' AS datetime)
)
INSERT dwh.DimProducts(ProductID,Product,ProductCategory,ProductSubCategory,ProductNumber,ProductModel,Color,StandardCost,ListPrice,Size,SizeUnitMeasureCode,Weight,StartDate,EndDate)
SELECT
  query.ProductID,
  query.Product,
