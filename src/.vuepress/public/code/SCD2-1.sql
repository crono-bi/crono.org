...
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='dwh' AND TABLE_NAME='DimProducts' AND COLUMN_NAME='FechaInicioVigencia')
ALTER TABLE dwh.DimProducts ADD FechaInicioVigencia datetime;

ALTER TABLE dwh.DimProducts ALTER COLUMN FechaInicioVigencia datetime;

IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='dwh' AND TABLE_NAME='DimProducts' AND COLUMN_NAME='FechaFinVigencia')
ALTER TABLE dwh.DimProducts ADD FechaFinVigencia datetime;

ALTER TABLE dwh.DimProducts ALTER COLUMN FechaFinVigencia datetime;
...
WHEN MATCHED AND (DimProducts.FechaFinVigencia=CAST('21000101' AS datetime)
...
WHEN NOT MATCHED BY SOURCE AND (DimProducts.FechaFinVigencia=CAST('21000101' AS datetime)) THEN
  UPDATE SET
    FechaFinVigencia=getdate();
...
target AS (
  SELECT ProductID
  FROM dwh.DimProducts
  WHERE DimProducts.FechaFinVigencia=CAST('21000101' AS datetime)
)
INSERT dwh.DimProducts(ProductID,Product,ProductCategory,ProductSubCategory,ProductNumber,ProductModel,Color,StandardCost,ListPrice,Size,SizeUnitMeasureCode,Weight,FechaInicioVigencia,FechaFinVigencia)
SELECT
  query.ProductID,
  query.Product,
  ...
  query.Weight,
  getdate(),
  CAST('21000101' AS datetime)
FROM query
LEFT JOIN target ON (target.ProductID=query.ProductID)
