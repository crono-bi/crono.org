...
**4. Sin repeticiones.** El código repetido es código que se desactualiza, que falla de formas distintas en sitios distintos y que nadie quiere mantener. Crono SQL elimina activamente las repeticiones: las columnas calculadas se reutilizan, los JOINs se simplifican, los patrones de carga se definen una sola vez. Y la misma sentencia que carga los datos define también la estructura de la tabla: el modelo y la carga nunca se dessincronizan porque son la misma cosa.
